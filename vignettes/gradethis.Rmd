...
To grade the exercise, we need to associate checking code with the exercise.
...
If a hypothetical student submitted `mean(1:10)` for their exercise code,
this checking code will return:

```{r average-check-manual, echo=FALSE}
grade_html(pass("Great work! You calculated the average of the first ten integers."))
```

If the student submits an incorrect answer, the default `fail()` message is encouraging:
...
1. `pass()` and `fail()` signal a final grade _as soon as they are called_.
   There are also [additional grade-signalling functions](#grading-helper-functions)
...
For convenience, gradethis also includes a few additional ojects.
...
You can reference any of these functions in your grading logic.
Additionally,
...
Be certain to include a final grade in case no other grades are returned.
Typically, 
you'll want to end your `grade_this()` grading code
with a single `pass()` or `fail()` statement.

### Feedback Messages

The `pass()` and `fail()` functions all use the `glue` package for custom message templating.
...
Each of these arguments — `praise`, `encourage`, and `hint` —
can be enabled or disabled via global options set by `gradethis_setup()`.
...
```{r pass-praise, eval=FALSE}
pass("That's exactly the average of 1 through 10.")
```

```{r pass-praise-feedback, echo=FALSE}
set.seed(1234567)
grade_html(
  grade_this({
    pass("That's exactly the average of 1 through 10.")
