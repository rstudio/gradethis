...
Sometimes we want to provide feedback for exercises that have multiple possible solutions.
With a little setup, gradethis can handle exercises with multiple solutions just as well as any other!
...
Use `filter()` or `drop_na()` to remove rows from the `starwars` dataset where `hair_color` is `NA`.

```{r starwars, exercise = TRUE}`r ''`
starwars %>%
  ____
```

```{r starwars-solution}`r ''`
# filter ----
starwars %>%
...
If a student gives an incorrect answer using `filter()`, code feedback will nudge them towards the correct `filter()` solution:

```{r filter_wrong, eval = FALSE}
starwars %>%
  filter(!hair_color == NA)
```
...
user_code <- paste(knitr::knit_code$get()$filter_wrong, collapse = "\n")

grade_html(grader(mock_this_exercise(!!user_code, !!solution_code)))
```

And if a student gives an incorrect answer using `drop_na()`, code feedback will nudge them towards the correct `drop_na()` solution:

```{r drop_na_wrong, eval = FALSE}
starwars %>%
  drop_na()
```

```{r echo = FALSE}
user_code <- paste(knitr::knit_code$get()$drop_na_wrong, collapse = "\n")
...
Remove rows from the `starwars` dataset where `hair_color` is `NA`.
...
grade_this({
  pass_if(
    is.null(code_feedback(solution_code = .solution_code_all[["filter"]])),
    "That's right, but an easier way to get the solution is with `{.solution_code_all[['drop_na']]}`."
  )
  pass_if_equal()
  fail(hint = TRUE)
})
...
user_code <- paste(knitr::knit_code$get()$filter_right, collapse = "\n")
