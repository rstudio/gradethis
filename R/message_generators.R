...
# bad argument name
bad_argument_name <- function(this_call, this, this_name = NULL) {
  ...
  glue::glue("{this_call} accepts more than one argument name that begins ",
...
# too_many_matches
too_many_matches <- function(this_call, that) {
  this_call <- prep(this_call)
  that <- prep(that)
  
  glue::glue("Double check the argument names you are using. ",
             "{this_call} accepts an argument named {that} and it ", 
